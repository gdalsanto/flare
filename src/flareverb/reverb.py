...
class BaseFDN(nn.Module):
    ...
    def get_params(self) -> OrderedDict[str, Any]:
        ...
        OrderedDict[str, Any]
            ...
            - 'onset_gain': Direct path gain values
    ...
    def _initialize_parameters(
        ...
        # Onset configuration
        self.onset_type = config.onset_type
    ...
    def _create_direct_path(self):
        ...
        self._configure_onset(onset_delay, direct_gain)
    ...
    def _configure_onset(self, onset_delay, direct_gain):
        """Configure onset behavior based on onset_type."""
        if self.onset_type is None:
            onset_delay.assign_value(
                onset_delay.sample2s(torch.zeros((self.in_ch,), device=self.device))
            )
            direct_gain.assign_value(torch.zeros((self.in_ch, 1)))

        elif self.onset_type == "random":
            self._configure_random_onset(onset_delay, direct_gain)

        elif self.onset_type == "FIR":
            raise NotImplementedError(
                "FIR onset type is not implemented yet. Please use 'random' or None."
            )
        else:
            raise ValueError(f"Unsupported onset type: {self.onset_type}")

    def _configure_random_onset(self, onset_delay, direct_gain):
        """Configure random onset behavior."""
        ...
        direct_gain.assign_value(torch.randn((self.in_ch, 1)))
